...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#btn_insert_report').click(function() {
			...
			var strCheckObjArr = [ "stdReportYear", "reportYear"];
			var sCheckObjNmArr = [ "기준연도", "생성연도"];
			if(gfnRequireCheck(strFormId, strCheckObjArr, sCheckObjNmArr)){
				return;	
			}
			
			if($("#stdReportYear").val()== $("#reportYear").val() ){
				jAlert("기준연도와 생성연도가 같습니다.", "알림창");
			}else{
				fnInsertAdm8000CopyInfo();	
			}
		...
		var fd = new FormData();
		function fnInsertAdm8000CopyInfo(){
			//FormData에 input값 넣기
			gfnFormDataAutoValue('admInsertForm',fd);
			//AJAX 설정
		
			var ajaxObj = new gfnAjaxRequestAction(
					{"url":"<c:url value='/adm/adm8000/adm8000/saveAdm8000CopyInfo.do'/>"
						,"contentType":false
						,"processData":false
						,"cache":false}
					,fd);
...
<div class="popup">
	...
	<div class="pop_title">보고서 기준 복사</div>
	<div class="pop_sub">
		<div class="pop_left">기준연도</span></div>
		<div class="pop_right">
			<span class="search_select">
				<select class="select_useCd" name="stdReportYear" id="stdReportYear" value="" style="height:100%; width:100%;"></select>
			</span>
		</div>
