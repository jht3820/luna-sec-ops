...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		/* 취소 */
		$('#btnPopStm1101Cancle').click(function() {
			gfnLayerPopupClose();
		});
		
		$('#btnPopStm1101Select').click(function() {
						...
						{display:true, addClass:"", style:"", list:[
							...
							{label:"검색", labelWidth:"50", type:"selectBox", width:"", key:"searchSelect", addClass:"", valueBoxStyle:"", value:"all",
								options:[
							        ...
							        {optionValue:'apiUrl', optionText:'URL'}                               
							...
							{label:"", labelWidth:"", type:"inputText", width:"120", key:"searchTxt", addClass:"secondItem sendBtn", valueBoxStyle:"padding-left:0px;", value:"",
								onkeyup:function(e){
									if(e.keyCode == '13' ){
										axdom("#" + popSearch.getItemId("btn_search_api")).click();
									}
								} 
							},
							...
							{label:"", labelWidth:"", type:"button", width:"55", key:"btn_search_api",valueBoxStyle:"padding-left:0px;padding-right:2px;", value:"<i class='fa fa-list' id='btn_search_api' aria-hidden='true'></i>&nbsp;<span>조회</span>",
							onclick:function(){
								/* 검색 조건 설정 후 reload */
	...
	function fnAxGridSet(){
        ...
        popGrid.setConfig({
            ...
            columns: [
				...
				{key: "apiNm", label: "서비스명", width: 200, align: "left"},
				{key: "apiUrl", label: "URL", width: 200, align: "left"},
	...
	function fnInGridPopListSet(ajaxParam){
		...
		var ajaxObj = new gfnAjaxRequestAction(
				{"url":"<c:url value='/stm/stm1000/stm1100/selectStm1100InfoAjax.do'/>","loadingShow":false}
				,ajaxParam);
		//AJAX 전송 성공 함수
	...
	function fnInSaveGrid(objects,prjId){
		...
		for(var i=0; i<objects.length; i++){
	 	...
	 	//AJAX 설정
		var ajaxObj = new gfnAjaxRequestAction(
				{"url":"<c:url value='/stm/stm1000/stm1100/saveStm1100Ajax.do'/>","loadingShow":true}
				,params);
		//AJAX 전송 성공 함수
		ajaxObj.setFnSuccess(function(data){
			data = JSON.parse(data);
...
<div class="popup">
	...
	<div class="pop_title">API 토큰 등록</div>
	<div class="pop_sub">
		<div class="tab_contents menu" style="max-width:600px;">
			<form:form commandName="api1100VO" id="searchFrm" name="searchFrm" method="post" onsubmit="return false"></form:form>
		...
		<div class="btn_div">
			<div class="button_normal save_btn" id="btnPopStm1101Select" >선택</div>
			<div class="button_normal exit_btn" id="btnPopStm1101Cancle" >취소</div>
		</div>
	</div>
	</form>
</div>
