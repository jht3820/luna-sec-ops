	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-12 kt-padding-b-10">
		<div class="kt-portlet kt-portlet--mobile kt-margin-b-0">
			...
			<div class="kt-portlet__body kt-padding-15 osl-min-h-px--575 osl-contents-frame">
				...
				<pre id="preBefore">
					<code class="osl-code-bg" id="stm8004BeforeFileInfo">
					</code>
				</pre>
			</div>
		</div>
	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-12 kt-padding-b-10">
		<div class="kt-portlet kt-portlet--mobile kt-margin-b-0">
			...
			<div class="kt-portlet__body kt-padding-15 osl-min-h-px--575 osl-contents-frame">
				...
				<pre id="preAfter">
					<code class="osl-code-bg" id="stm8004AfterFileInfo">
					</code>
				</pre>
			</div>
		</div>
		...
		//AJAX 설정
   		...
   		ajaxObj.setFnSuccess(function(data){
   			if(data.errorYn == "Y"){
				...
				$("#stm8004BeforeFileInfo").html(data.message);
				$("#stm8004AfterFileInfo").html(data.message);
				...
				if($.osl.isNull(beforeContent)){
					$("#stm8004BeforeFileInfo").html("<span class='kt-font-inverse-brand  kt-padding-l-10 osl-font-lg osl-font'>"+"로그가 없습니다."+"</span>");
					return false;
				}
	   			
				// 로그가 없을 경우
				if($.osl.isNull(afterContent)){
					$("#stm8004AfterFileInfo").html("<span class='kt-font-inverse-brand  kt-padding-l-10 osl-font-lg osl-font'>"+"로그가 없습니다."+"</span>");
					return false;
				}
				
				$("#stm8004BeforeFileInfo").html($.osl.escapeHtml(beforeContent));
				$("#stm8004BeforeFileInfo").each(function(i, block) {hljs.highlightBlock(block);});
				$("#stm8004AfterFileInfo").html($.osl.escapeHtml(afterContent));
				$("#stm8004AfterFileInfo").each(function(i, block) {hljs.highlightBlock(block);});
				...
				var diffs = dmp.diff_main($('#stm8004BeforeFileInfo').html(), $('#stm8004AfterFileInfo').html());
				...
				$("#stm8004BeforeFileInfo").html(oldVal);
				$("#stm8004AfterFileInfo").html(newVal);
				...
				if(!$.osl.isNull(oldVal)){
					...
					//코드 창 크기 높이
					var codeWindowHeight = $("#stm8004BeforeFileInfo").height() - 50;
				...
				if(!$.osl.isNull(newVal)){
					...
					//코드 창 크기 높이
					var codeWindowHeight = $("#stm8004AfterFileInfo").height() - 50;
				...
				$("#stm8004BeforeFileInfo").prepend($("#codeLineFrameBefore"));
				$("#stm8004AfterFileInfo").prepend($("#codeLineFrameAfter"));
				
				//가로 스크롤 동시 제어
				$("#stm8004BeforeFileInfo").scroll(function(){
					//우측 가로,세로 스크롤 제어
					$('#stm8004AfterFileInfo').scrollTop($("#stm8004BeforeFileInfo").scrollTop());
					$('#stm8004AfterFileInfo').scrollLeft($("#stm8004BeforeFileInfo").scrollLeft());
					
					//코드 라인 스크롤 제어
					$('#codeLineFrameBefore').scrollTop($("#stm8004BeforeFileInfo").scrollTop());
				});
				$("#stm8004AfterFileInfo").scroll(function(){
					
					//코드 라인 스크롤 제어
					$('#codeLineFrameAfter').scrollTop($("#stm8004AfterFileInfo").scrollTop());
				});
   			}
   		});
  	 	//AJAX 전송
